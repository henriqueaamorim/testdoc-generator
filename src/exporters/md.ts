import type { NormalizedDocument } from "../shared/types.js";

const esc = (s: string | undefined) => (s ?? "").replace(/\|/g, "\\|");

export function exportToMarkdown(doc: NormalizedDocument): string {
	const lines: string[] = [];
	...
	const toBr = (iso: string) => (iso ? `${iso.slice(8, 10)}/${iso.slice(5, 7)}/${iso.slice(0, 4)}` : "");
