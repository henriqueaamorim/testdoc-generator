...
const Ajv = Ajv2020Default;
const addFormats = addFormatsDefault;
const ajv = new Ajv({ allErrors: true, strict: false, validateSchema: false });
addFormats(ajv);
const schemaForAjv = { ...testDocSchema };
if (schemaForAjv.$schema)
...
app.post("/v1/export", (req, res) => {
    ...
    const ok = validateDoc(document);
    if (!ok)
        return res.status(400).json({ error: "invalid document", details: validateDoc.errors });
